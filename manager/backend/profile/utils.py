...
from backend.gamification.models import GameProfile
...
def get_profile_info_serialized(requester, pk):
    ...
    if requester == user:
        ...
        result['coin_to_usd'] =human_format(get_converted_coins(result['tagg_score']))
...
def reset_today_score():
    logger.info("reset Started")
    reset_data = GameProfile.objects.filter(today_score__gt = 0).update(today_score = 0)
