...
def increase_tagg_score(user, score_earned):
    try:
        ...
        if game_profile:
            ...
            game_profile.today_score = game_profile.today_score + score_earned
            game_profile.tier = determine_gamification_tier(game_profile.tagg_score)
            game_profile.save()
        # today = datetime.now()
        # if (today == game_profile.timestamp):
        #     # old logic
        #     game_profile.today_score = game_profile.today_score + score_earned
        
        # else:
        #     game_profile.timestamp = today
        #     game_profile.today_score =  score_earned
...
def decrease_tagg_score(user, score_remove):
    try:
        ...
        if game_profile:
            ...
            game_profile.today_score = game_profile.today_score - score_remove
            game_profile.tier = determine_gamification_tier(game_profile.tagg_score)
            game_profile.save()
    except TaggTierException:
        raise TaggTierException
...
def determine_gamification_tier(score):
    # tma-2171, tma-2070 and tma-2031 Update tier for the user based on tagg score
    try:
