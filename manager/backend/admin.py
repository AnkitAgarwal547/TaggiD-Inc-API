        ...
        if inviteObj:
            ...
            for items in inviteObj:
                ...
                if game_profile_inviter:
                    game_profile_inviter.tagg_score = game_profile_inviter.tagg_score+2
                    game_profile_inviter.today_score = game_profile_inviter.today_score+2
                    game_profile_inviter.save()
                    
        game_profile_invited = GameProfile.objects.filter(tagg_user=user).first()
        if game_profile_invited:
            game_profile_invited.tagg_score = game_profile_invited.tagg_score+2
            game_profile_invited.today_score = game_profile_invited.today_score+2
    ...
    try:
        for user in queryset:
            if not GameProfile.objects.filter(tagg_user=user).exists():
                GameProfile.objects.create(tagg_user=user, tagg_score=10)
    except Exception:
