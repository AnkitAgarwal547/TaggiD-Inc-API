...
class EditProfileViewSet(viewsets.ViewSet):
    ...
    def partial_update(self, request, pk=None):
        try:
            ...
            # tma1672 skin object for detecting active skin for user
            active_skin = Skin.objects.filter(owner__id=pk, active=True).first()
            ...
            files = request.FILES
            # tma1672 for a user has profile 3 Cinema as active skin, update header image and profile image both when user changes header image
            if active_skin.template_type == TemplateType.THREE:
                for image_header in files:
                    if image_header not in VALID_IMAGE_KEYS:
                        continue
                    for image_name in VALID_IMAGE_KEYS:
                        try:
                            image = Image.open(files.get(image_header), "r")
                            legal_image(image)
                            upload_image(
                                image,
