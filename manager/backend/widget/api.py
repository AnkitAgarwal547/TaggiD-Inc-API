...
from ..gamification.utils import increase_tagg_score, decrease_tagg_score, determine_gamification_tier, TaggTierException, TaggScoreNotSufficient, TaggScoreUpdateException
...
class WidgetTypeBaseViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # decreases tagg score for adding a widget
        try:
            # remove decrease until bug is fixed where app can handle 400 error messages
            #increase_tagg_score(request.user, TAGG_SCORE_ALLOTMENT["TAGG_CREATE"])
            decrease_tagg_score(request.user, TAGG_SCORE_ALLOTMENT["TAGG_CREATE_REMOVE"])
            ...
            return Response(
                "Tagg score update failure", status=400
            )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if (
            ...
            if request.data["thumbnail_url"] != "" or request.data["thumbnail_url"]:
                #tma2087 and tma2101
                tagg_edit_flag = True
            else:
                request.data[
        ...
        if rewardObj:
            if rewardObj.count > 30:
                ...
                if gameObj:
                    ...
                    gameObj.today_score = gameObj.today_score + 5
                    rewardObj.count = 0
                    rewardObj.save()
                    gameObj.save()
                    # tma-2031
...
class RewardsAdd(ModelViewSet):
    ...
    def create(self, request):
        ...
        if gameObj:
            ...
            gameObj.today_score = gameObj.today_score +int(coins)
            gameObj.save()
            # tma-2031
            gameObj.tier = determine_gamification_tier(gameObj.tagg_score)
            gameObj.save()
    ...
    def create(self, request):
        try:
            ...
            if (
                ...
                gameObj.today_score = gameObj.today_score - 20
                gameObj.save()
                # tma-2031
                gameObj.tier = determine_gamification_tier(gameObj.tagg_score)
                gameObj.save()
