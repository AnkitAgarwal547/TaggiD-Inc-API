...
from ..analytics.widgets.utils import (
    record_widget_click_no_notif,
)
from ..moments.views.utils import record_moment_view
from ..analytics.profile.utils import record_profile_click
...
def widget_view_boost():
    ...
    user = TaggUser.objects.filter(taggusermeta__is_onboarded=True).first()
    for widget in Widget.objects.filter(active=True):
        try:
            record_widget_click_no_notif(widget, user)
    ...
    #     try:
    #         record_moment_view(momentObj, user)
    #     except Exception as error:
    #         logger.exception(error)
    #         logger.error(
    ...
    #         )
    #         continue
    logger.info("Started profile_view_boost")
    for singleUser in TaggUser.objects.filter(taggusermeta__is_onboarded=True):
        try:
            record_profile_click(singleUser, user)
        except Exception as error:
            logger.exception(error)
            logger.error(
                f"Failed to record profile view for user: {user.username}"
            )
            continue
