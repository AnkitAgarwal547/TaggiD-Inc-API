...
from ..common.number_formatter import human_format
...
class MomentPostSerializer(serializers.ModelSerializer):
    ...
    def get_comments_count(self, obj):
        ...
        commentCount= get_moment_comments_count(obj.moment_id, self.context.get("user"))
        return human_format(commentCount)

    def get_user(self, obj):
        return TaggUserSerializer(obj.user_id).data

    def get_view_count(self, obj):
        viewsCounts=MomentViews.objects.filter(moment_viewed=obj.moment_id).count()
        return human_format(viewsCounts)

    def get_share_count(self, obj):
        shareCount=MomentShares.objects.filter(moment_shared=obj.moment_id).count()
        return human_format(shareCount)
